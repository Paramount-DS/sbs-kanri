...
const DASHBOARD_FIELDS = {
  ...
  usage: [
    ...
    ["usageDays","利用日数","number"],["usedFeatures","利用機能数","number"],["totalFeatures","全機能数","number"],
...
function dashboardCalculations(project) {
  ...
  const monthlySaved=(numberValue(roi.monthlySavedHours)!==null && numberValue(roi.hourlyLaborCost)!==null) ? numberValue(roi.monthlySavedHours)*numberValue(roi.hourlyLaborCost) : null;
  ...
  const newestVisit=latestVisitDate(project) || activity.lastVisitDate;
  ...
  return {activeRate,featureRate,connectionRate,systemUsage,kpiRate,monthlySaved,annualSaved,roiRate,payback,newestVisit,nextContact,visitElapsed,renewalDays,unresolved,health,missing};
}
...
function categoryRows(category,p,c) {
  ...
  if(category==="csActivity") return [["最終訪問日",shown(c.newestVisit)],["経過日数",shown(c.visitElapsed,"日")],["訪問回数",shown(x.visitCount||p.visits?.length||0)],
    ["定例会回数",shown(x.regularMeetingCount)],["問い合わせ件数",shown(x.inquiryCount)],["未解決課題数",shown(x.unresolvedIssues)],
    ["教育実施回数",shown(x.trainingCount)],["最終対応日",shown(c.newestVisit||x.lastContactDate)],["次回対応予定日",shown(c.nextContact)]];
  if(category==="usage") return [["システム利用率",percent(c.systemUsage)],["アクティブ率",percent(c.activeRate)],["機能活用率",percent(c.featureRate)],
    ...
    ["利用日数",shown(x.usageDays)],["アラート件数",shown(x.alertCount)],["データ閲覧回数",shown(x.dataViews)],["ダッシュボード閲覧回数",shown(x.dashboardViews)]];
  ...
  if(category==="roi") return [["契約金額",shown(x.contractAmount,"円")],["月間削減時間",shown(x.monthlySavedHours,"時間")],["人件費単価",shown(x.hourlyLaborCost,"円")],
...
function renderSummary(p,c) {
  ...
  const dailySaved=numberValue(data.roi.monthlySavedHours)===null?"-":(numberValue(data.roi.monthlySavedHours)/30).toFixed(1);
  const items=[
...
function renderVisitsCard(p) {
  const visits=[...(p.visits||[])].sort((a,b)=>(b.endDate||b.startDate||"").localeCompare(a.endDate||a.startDate||""));
  return `<div class="dashboard-visit-summary"><strong>${visits.length}</strong><span>累計記録数</span></div>${visits.slice(0,5).map(v=>`<div class="dashboard-visit-row"><time>${escapeHtml(v.endDate||v.startDate||"-")}</time><div><strong>${escapeHtml(getVisitStatusLabel(v.status))}</strong><span>${escapeHtml(v.taskItem||v.freeText||"-")}</span></div></div>`).join("")||'<p class="dashboard-empty">訪問記録がありません</p>'}`;
}
...
function renderUsagePanel(p,c) {
  const data=getDashboardData(p).usage;
  const rows=[["システム利用率",c.systemUsage],["アクティブ率",c.activeRate],["機能活用率",c.featureRate],["端末接続率",c.connectionRate],["ログイン回数",numberValue(data.loginCount)],["利用日数",numberValue(data.usageDays)],["データ閲覧数",numberValue(data.dataViews)]];
...
function renderCategories(p,c) {
  document.getElementById("dashboardCategoryGrid").innerHTML=[
    ...
    dashboardPanel("visits","訪問履歴","直近の対応記録",renderVisitsCard(p),c),
...
function usageForm(data) {
  const fields=DASHBOARD_FIELDS.usage.map(field=>inputHtml(field,data[field[0]])).join("");
...
function visitsForm(p) {
  ...
  return `<div class="dashboard-visit-editor-list">${visits.map((v,i)=>`<div><span>${escapeHtml(v.endDate||v.startDate||"-")} / ${escapeHtml(getVisitStatusLabel(v.status))}</span><button type="button" onclick="editDashboardVisit(${i})">編集</button><button type="button" class="danger" onclick="deleteDashboardVisit(${i})">削除</button></div>`).join("")||'<p>履歴はありません</p>'}</div><hr><input type="hidden" name="visitIndex" value="${dashboardVisitEditIndex}">${inputHtml(["visitDate","対応日","date"],"")}${inputHtml(["visitType","対応種別","text"],"")}${inputHtml(["person","対応者","text"],"")}${inputHtml(["counterpart","面談相手","text"],"")}<div class="form-group"><label class="form-label">内容</label><textarea class="form-textarea" name="content" rows="3"></textarea></div><div class="form-group"><label class="form-label">次回アクション</label><textarea class="form-textarea" name="nextAction" rows="2"></textarea></div>${inputHtml(["nextDate","次回対応予定日","date"],"")}`;
}
function openDashboardModal(category) {
  dashboardEditingCategory=category; dashboardVisitEditIndex=-1;
  const title=category==="phase"?"現在フェーズ":CATEGORY_META[category][0];
  document.getElementById("dashboardModalTitle").textContent=`${title} Input`;
...
function closeDashboardModal(){ document.getElementById("dashboardEditModal").classList.remove("open"); dashboardEditingCategory=""; dashboardVisitEditIndex=-1; }
...
async function saveDashboardVisit(values){
  if(!values.visitDate) throw new Error("対応日を入力してください");
  const visits=[...(dashboardProject.visits||[])], entry={startDate:values.visitDate,endDate:values.visitDate,status:values.visitType||"対応",freeText:values.content||"",dashboardVisit:{person:values.person||"",counterpart:values.counterpart||"",nextAction:values.nextAction||"",nextDate:values.nextDate||""},createdAt:new Date().toISOString()};
  if(dashboardVisitEditIndex>=0) visits[dashboardVisitEditIndex]={...visits[dashboardVisitEditIndex],...entry}; else visits.push(entry);
  await dashboardDocRef.update({visits});
}
function editDashboardVisit(index){ dashboardVisitEditIndex=index; const v=dashboardProject.visits[index], form=document.getElementById("dashboardEditForm"); form.elements.visitIndex.value=index; form.elements.visitDate.value=v.endDate||v.startDate||""; form.elements.visitType.value=v.status||""; form.elements.person.value=v.dashboardVisit?.person||""; form.elements.counterpart.value=v.dashboardVisit?.counterpart||""; form.elements.content.value=v.freeText||""; form.elements.nextAction.value=v.dashboardVisit?.nextAction||""; form.elements.nextDate.value=v.dashboardVisit?.nextDate||""; }
async function deleteDashboardVisit(index){ const visits=[...(dashboardProject.visits||[])]; visits.splice(index,1); try{await dashboardDocRef.update({visits}); openDashboardModal("visits"); showToast("削除しました");}catch(e){showToast("削除に失敗しました","error");} }
...
async function migrateDashboard(snapshot){
  ...
  if(!needs || migrationRunning) return;
  migrationRunning=true;
  try{await snapshot.ref.set({csDashboard:merged},{merge:true});}finally{migrationRunning=false;}
}
