...
function basicRows(p) {
  ...
  return [["院名",shown(p.hospitalName)],["備考",shown(p.memo)],["都道府県",shown(p.prefecture)],["案件タイプ",shown(projectType)],["導入病棟名",shown(p.ward)],["稼働開始日",shown(p.startDate)],["サポートエンド",shown(p.supportEndDate)],
...
function basicForm(p) {
  ...
  const fields=[["hospitalName","院名","text"],["prefecture","都道府県","select",[["","-"],...CS_PREFECTURES.map(value=>[value,value])]],["projectType","案件タイプ","select",projectTypes],["ward","導入病棟名","text"],["startDate","稼働開始日","date"],["supportEndDate","サポートエンド","date"],
    ...
    ["bedsideTerminal","ベッドサイド端末（既存/新規台数）","text"],["stationTerminal","ステーション端末（既存/新規台数）","text"],["nemiriScan","眠りSCAN（既存/新規台数）","text"],["rishoCatch","離床CATCH（既存/新規台数）","text"],["wifiNav","Wi-Fiベッドナビ（既存/新規台数）","text"],["cameraCount","カメラ（数量）","text"],["mobileCount","モバイル（数量）","text"],["tabletPos","タブレット設置位置","text"],
    ["electronicKarte","電子カルテ（ベンダー/機種）","text"],["nurseCall","ナースコール（メーカー/機種）","text"],["shuhenRenkei","周辺連携機能","text"],["ankenGaiyou","案件概要","text"]];
  const memoField=`<div class="form-group"><label class="form-label">備考</label><textarea class="form-textarea" name="memo" rows="4" placeholder="基本情報に関する備考を入力してください">${escapeHtml(p.memo||"")}</textarea></div>`;
  return fields.map((f,index)=>inputHtml(f,p[f[0]])+(index===0?memoField:"")).join("")+`<div class="form-group"><label class="form-label">導入製品</label><div class="dashboard-check-grid">${[["hasBedside","BS端末"],["hasBedNavi","ベッドナビ"],["hasNemiri","眠りSCAN"],["hasRisha","離床CATCH"],["hasVital","バイタル連携"],["hasEhr","EHR連携"],["hasNurse","NC情報連携"],["hasNcNotify","NC通知連携"],["hasCamera","カメラ"],["hasMobile","モバイル"]].map(([k,l])=>`<label><input type="checkbox" name="${k}"${p[k]?" checked":""}>${l}</label>`).join("")}</div></div>${inputHtml(["quickMemo","簡易メモ","text"],p.quickMemo)}`;
}
function visitsForm(p) {
  const visits=(p.visits||[]).map((visit,index)=>({visit,index})).sort((a,b)=>(b.visit.endDate||b.visit.startDate||"").localeCompare(a.visit.endDate||a.visit.startDate||""));
  return `<div class="dashboard-visit-editor-list">${visits.map(({visit,index})=>`<div><span>${escapeHtml(visit.endDate||visit.startDate||"-")} / ${escapeHtml(getVisitStatusLabel(visit.status))}<small>${escapeHtml(normalizeCsActivityItem(visit.taskItem)||visit.freeText||"-")}</small></span><button type="button" onclick="openDashboardVisitEditor(${index})">Edit</button><button type="button" class="danger" onclick="deleteDashboardVisit(${index})">削除</button></div>`).join("")||'<p class="dashboard-empty">訪問履歴はありません</p>'}</div>`;
}
...
async function saveDashboardForm(event) {
  ...
  try {
    if(category==="basic") {
      ["hasBedside","hasBedNavi","hasNemiri","hasRisha","hasVital","hasEhr","hasNurse","hasNcNotify","hasCamera","hasMobile"].forEach(k=>values[k]=form.elements[k]?.checked||false);
...
async function saveDashboardVisit(values){
  ...
  visits[dashboardVisitEditIndex]={...current,status:normalizeVisitStatus(values.visitStatus),taskItem:task?.item||values.taskItem||"",taskContent:task?.content||"",taskEffect:task?.effect||"",assignee:(values.assignee||"").trim(),isVisit:values.isVisit==="on",startDate:values.startDate||values.endDate,endDate:values.endDate||values.startDate,freeText:values.freeText||"",updatedAt:new Date().toISOString()};
